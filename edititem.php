...
<head>
  ...
  <title>Update Item -- Food.INC</title>
...
<body id="page-top">
  ...
  $id = intval($_GET['id']);
  $result = mysqli_query($con, "SELECT * FROM item_table WHERE item_id = $id");
  $row = mysqli_fetch_array($result);
  $result2 = mysqli_query($con, "SELECT * FROM cuisine_table");
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Cuisine</h1><br>
          <h2 class="h4 mb-4 text-gray-700">Edit Cusine</h2>
          ...
          <div class="card shadow mb-4">
            ...
            <div class="card-body">
              <form action="php/update_item.php" method="post" enctype="multipart/form-data">
                <p>Item Name:</p>
                <input class="card col-sm-12 form-control " type="text" name="item_name" required="required" value='<?php echo $row['item_name'] ?>'><br>
                <p>Item's Cuisine</p>
                <select name="item_cuisine" class="card">
                  <?php
                  while ($row2 = mysqli_fetch_array($result2))
                  {
                  echo "<option value='".$row2['cuisine_id']."'";
                  if ($row['cuisine_id'] == $row2['cuisine_id'])
                  {
                    echo " selected>".$row2['cuisine_type']."</option>";
                    }
                  else {
                  echo ">".$row2['cuisine_type']."</option>";
                  }
                  }
                  ?>
                </select>
                <p>Item Description:</p>
                <textarea class="card col-sm-12 form-control" name="item_description" required="required" style="height:100px;"><?php echo $row['item_description'] ?></textarea><br>
                <p>Item Price</p>
                <input class="card col-sm-12 form-control " type="text" name="item_price" required="required" value='<?php echo $row['item_price'] ?>'><br>
